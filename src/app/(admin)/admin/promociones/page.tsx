...
export default async function AdminPromocionesPage() {
  ...
  const { items: promotions, total } = await listAllPromotions()
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{total} promoción{total !== 1 ? 'es' : ''} en total</p>
        </div>
        <Button asChild>
          <Link href="/admin/promociones/nueva">+ Nueva promoción</Link>
        </Button>
      </div>
              ...
              <TableRow key={p.id}>
                ...
                <TableCell className="text-right">
                  <Button variant="ghost" size="sm" asChild>
                    <Link href={`/admin/promociones/${p.id}`}>Ver</Link>
                  </Button>
                </TableCell>
              </TableRow>
            ))}
