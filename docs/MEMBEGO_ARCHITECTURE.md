...
| **Benefit Transformation Engine** | ✅ Construido (E1.7) | `src/lib/benefit-transformation` — motor universal de transformación de beneficios: upgrade, downgrade, exchange, replacement, customization (split/merge preparados). Resolution Engine (pipeline de fondeo: promos, cupones, puntos, créditos, campañas, VIP, cumpleaños, pago). Políticas configurables por empresa. 10 plantillas Car Wash. |
...
| **Automation Engine** | ✅ Construido (FE1) + **conectado a la app** | `src/lib/automation` — motor universal: trigger + condiciones (Rule/BEL) + acciones (Action Engine) + esperas + variables + eventos + encadenado + auditoría/métricas. Conectado en vivo vía `src/modules/estrategias`: `emitirEventoEstrategia` (bus fire-and-safe desde registro, visitas, activación de pago y referidos) + `LiveActionSink` (notificación in-app, email, benefit_grants, eventos, run_workflow; canales no integrados quedan como intención). |
| **Automation Playbooks** | ✅ E1.1–E1.10 (completa) | `src/lib/automation/playbooks` — estrategias comerciales instalables (24 apartados del Documento Maestro) que envuelven automatizaciones reales de E1. E1.1: 20 captación (`ACQ-*`). E1.2: 18 onboarding (`ONB-*`). E1.3: 12 primera compra (`FP-*`). E1.4: 16 frecuencia (`FREQ-*`). E1.5: 16 recuperación (`REC-*`). E1.6: 20 membresías (`MEM-*`). E1.7: 20 referidos (`REF-*`). E1.8: 20 campañas (`CAMP-*`). E1.9: 20 gamificación conductual (`GAM-*`). E1.10: 18 decisiones inteligentes (`DEC-*`, sobre la arquitectura de decisiones). **180 playbooks** universales y editables. Sin migración: reutiliza las tablas de E1. |
| **Platform Registry** | ✅ Construido (FE2) | `src/lib/platform` — registro único de plataforma: catálogo de eventos (80, con `EVENT_ALIASES` canónicos), acciones (47, fuente Action Engine) y métricas unificadas (fusión deduplicada de los 4 catálogos con procedencia). Ver `docs/MEMBEGO_CONSOLIDATION_E2.md` (12 entregables, ADRs y backlog). |
| **Decision Engine + Providers** | ✅ Construido (FE1.10) | `src/lib/decision` — arquitectura de decisiones DESACOPLADA: Decision Engine (decide, nunca ejecuta) → Recommendation Engine → Prediction Framework → Optimization Engine, servida por Decision Providers. Único proveedor hoy: **Rule Based**; preparada para añadir AI/ML/Custom sin tocar nada. Las empresas nunca ven "IA". |
...
| **Recommendation Engine** | ✅ Construido (FE1.10) | `src/lib/decision` — produce recomendaciones a partir de decisiones; nunca ejecuta (eso es del Action Engine). |
